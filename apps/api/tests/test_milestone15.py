...
async def test_oidc_authentication_validation(mock_fetch_jwk, rsa_keys) -> None:
    ...
    # Generate token
    token = jwt.encode(
        payload, pem_private, algorithm="RS256", headers={"kid": "test-key-id"}
    )
    ...
    try:
        ...
        with patch(
            ...
            side_effect=AuthenticationError("No API Key match"),
        ):
            user_context = await get_current_user(
                _security_request(), f"Bearer {token}"
            )
            ...
            payload_expired["exp"] = (
                int(datetime.datetime.now(datetime.UTC).timestamp()) - 100
            )
            ...
            with pytest.raises(HTTPException) as exc:
                await get_current_user(_security_request(), f"Bearer {token_expired}")
...
def test_data_retention_scheduler_purging(mock_get_engine) -> None:
    ...
    tenant_id = str(uuid.uuid4())
    config_dict = {"security_settings": {"data_retention_ttl_days": 15}}
