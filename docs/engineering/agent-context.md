...
| **RAG Audit Execution** | [rag-audit-report.md](file:///Users/prateeksharma/Developer/retriever/docs/engineering/rag-audit-report.md) | Target files from §5 (Appendix) | Read item from §3 → find file from §5 → implement per §4 checklist → run tests |
| **Retrieval Pipeline Work** | [bm25-two-stage.md](file:///Users/prateeksharma/Developer/retriever/docs/engineering/bm25-two-stage.md) | 1. `domain/retrieval/` (search_service, bm25_reranker, mmr)<br>2. `domain/abstractions/retrieval.py` (ports) | Read BM25 doc → check existing port contracts → modify search_service pipeline → verify latency budget < 150ms |
| **Evaluation Framework Work** | [rag-audit-report.md](file:///Users/prateeksharma/Developer/retriever/docs/engineering/rag-audit-report.md) §2.5 | 1. `domain/abstractions/evaluation.py` (ports)<br>2. `domain/evaluation/` (evaluator, search_metrics)<br>3. `adapters/database/evaluation_repository.py` | Read eval port → modify evaluator → run eval via admin API → check results in eval_runs table |
| **Corrective Retrieval Work** | [rag-audit-report.md](file:///Users/prateeksharma/Developer/retriever/docs/engineering/rag-audit-report.md) P-2 | 1. `domain/abstractions/retrieval.py` (CorrectiveRetrievalProvider + Decision)<br>2. `adapters/cognitive/corrective_retrieval_adapter.py`<br>3. `domain/retrieval/corrective_retrieval_service.py` | Read config defaults → wire new port → write judge adapter → create service wrapping search+generate → wire into main.py route handler → run tests |
