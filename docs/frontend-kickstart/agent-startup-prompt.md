...
- **Headless Backend:** The Retriever RAG backend is a high-performance FastAPI engine deployed on Oracle Cloud VPS at `http://localhost:8000` with Supabase (PostgreSQL + pgvector) and local Ollama embeddings (`nomic-embed-text`, 768-dim).
...
- **Dynamic System Prompts & Intent:** Do not hardcode system prompts in the client application code. Prompts are managed via the Retriever Admin Dashboard (`http://localhost:3000`). The frontend calls the message creation endpoint:
