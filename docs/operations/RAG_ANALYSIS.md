...
The form validates both Tenant ID and User ID with a strict UUID regex ([RagInterface.tsx:23](#L23)). UUIDs like `a8b819bb-61bb-450b-9662-62bd06b188d3` are:
...
| 6 | [RagInterface.tsx]() | Heavy use of `any` type (6 `eslint-disable` comments). `results`, `docs`, and `doc` all untyped. | Define interfaces: `SearchResult`, `DocumentMeta`, `SearchResponse`. |
...
| 11 | [RagInterface.tsx:90](#L90) | Default config hardcodes `tenantId: "00000000-..."` and `userId: "a8b819bb-..."`. These are production data leaks, not sensible defaults. | Start with empty strings. |
...
| # | Location | Issue |
|---|----------|-------|
| 17 | [RagInterface.tsx:7-14](#L7-L14) | Deferred features list (markdown rendering, session history, stop button, keyboard shortcuts) is reasonable. These are clearly marked and not blocking. |
...
| [Removed — docker.yml](../../.github/workflows/ci.yml) | Push/PR to main | Build + push API and Worker Docker images to GHCR — **removed (not actively used)** | ✅ N/A — file deleted |
...
|------------|--------|----------|
| Real deployment topology documented | ✅ Done | [DEPLOYMENT.md](../infrastructure/DEPLOYMENT.md) and [ORACLE_DEPLOYMENT_REFERENCE.md](../infrastructure/ORACLE_DEPLOYMENT_REFERENCE.md) exist |
