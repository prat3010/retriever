...
curl -s -H "X-Admin-Master-Key: $ADMIN_MASTER_KEY" \
  http://localhost:8000/v1/admin/serverless/status | jq .
```
...
```bash
curl -X POST "http://localhost:8000/v1/admin/serverless/probe" \
...
```bash
curl -s -H "X-Admin-Master-Key: $ADMIN_MASTER_KEY" \
  "http://localhost:8000/v1/admin/serverless/cost-savings?active_compute_hours=15.0&gpu_tier=A10G" | jq .
```
...
```bash
curl -X POST "http://localhost:8000/v1/tenants/$TENANT_ID/lora-adapters" \
...
```bash
curl -X POST "http://localhost:8000/v1/tenants/$TENANT_ID/lora-adapters/$ADAPTER_ID/activate" \
...
```bash
curl -X POST "http://localhost:8000/v1/tenants/$TENANT_ID/lora-adapters/$ADAPTER_ID/deactivate" \
  -H "Authorization: Bearer $TENANT_API_KEY" | jq .
```
